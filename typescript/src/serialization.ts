...
function parsePrompt(value: JsonValue): Prompt {
  const node = object(value, "prompt"); const known = new Set(["id", "label", "response", "role", "language", "hints"]);
  const hints = optionalObject(node.hints, "hints");
  return { id: string(node, "id", "prompt") ?? "", label: string(node, "label", "prompt") ?? "", response: string(node, "response", "prompt") ?? "", role: string(node, "role", "prompt"), language: string(node, "language", "prompt"), hints: hints ? parseHints(hints) : { suggestedValues: [], extra: {} }, extra: rest(node, known), responseIsDeclared: "response" in node };
}
function parseSection(value: JsonValue): Section {
  const node = object(value, "section"); const known = new Set(["id", "title", "description", "kind", "canAddRows", "maxRows", "role", "language", "prompts", "sections"]);
