...
public partial class MainShellViewModelTests
{
    ...
    public async Task OpenFromPath_ForFilling_ShowsTemplateAsFormInsteadOfEditor()
    {
        ...
        try { await File.WriteAllTextAsync(file, "{\"aprVersion\":\"1.0-beta.6\",\"documentType\":\"template\",\"metadata\":{\"title\":\"T\"},\"sections\":[{\"id\":\"s\",\"title\":\"S\",\"prompts\":[{\"id\":\"p\",\"label\":\"Name\",\"response\":\"\"}]}]}"); var shell = CreateShell(); await shell.OpenFromPath(file, openForFilling: true); shell.IsEditMode.Should().BeFalse(); shell.ShowFullFillList.Should().BeTrue(); } finally { File.Delete(file); }
    }
