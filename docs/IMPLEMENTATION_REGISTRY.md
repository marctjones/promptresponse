...
| .NET Core | Reference core, CEL, representations, streams, attestations | Beta.6 core+attestations, manifest resolution, CMS verification, and independent attestation creation | **scored 171/171** against the blinded document suite, plus beta.6 core tests |
| Desktop | Author, fill, review, export, import, attestation display | Beta.6 JSONC/YAML form I/O plus stream-occurrence browser; attestation states are non-gating and trust remains external by design | **scored 15/15** against the renderer suite from the shipped shell booted headlessly, plus GUI/accessibility/desktop tests |
| CLI | Validate, fill, review, eval, import/export, stream and attestation inspection | Every runtime command uses beta.6-only form I/O; validate/info/normalize/attest use stream APIs, and retired embedded-signature commands are rejected | **scored 171/171** as its own driver, so a defect in the command line's own layer is distinguishable from one in the library; plus CLI tests and release smoke |
| PDF renderer/importer | Flat/fillable/PDF-A export and AcroForm import | Shipped with limits | **scored** against the renderer suite's exporter surface, which is the only place `APR-RENDER-009` can be asked; plus PDF tests |
| Python SDK and local web demo | Core reading/writing and local demo | Beta.6 core+attestations with digest/manifest/witness resolution and detached CMS content verification; trust is supplied by the caller | **scored 153/153** on `core`, `core+streams` and `core+expressions` against the blinded document suite (`core+attestations` unclaimed, matching .NET); plus SDK tests (#379) |
| TypeScript SDK/HTML projection | Browser-capable core and renderer | Beta.6 core+attestations with async detached CMS content verification and resolution; trust is supplied by the caller | **scored 153/153** on `core`, `core+streams` and `core+expressions` against the blinded document suite (`core+attestations` unclaimed, matching .NET and Python); plus SDK tests (#379) |
| Java SDK and local demo | Core processing and local JDK demo | Beta.6 core+attestations with digest/manifest/witness resolution and detached CMS content verification; trust is supplied by the caller | *exercised* against the shared corpus and the specification's examples — not yet scored against the blinded document suite (#379) |
